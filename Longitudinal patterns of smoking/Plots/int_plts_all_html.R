...
explt_int <- function(df){
  ...
  df$x <- 1:nrow(df)
  ggplot(df, aes(x = x, y = log_OR, ymin = log_LCI, ymax = log_UCI, colour = sub_grp, size = n, text = text)) +
    ...
    guides(colour=guide_legend(nrow=4, byrow=TRUE, override.aes = aes(label = "")),
           ...
           size=guide_legend(nrow=4, byrow=TRUE, override.aes = aes(label = ""))) +
    labs(x = '', y = "log(Odds ratio)", colour = 'Category')
}
